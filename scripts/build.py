...
def rewrite_rss_article(title: str, body_text: str, category: str) -> str | None:
    """
    Rewrite the actual RSS article body text using Groq Llama 3.

    When body_text is available (200+ chars):
      - Rewrites paragraph-by-paragraph
      - Preserves sentence count per paragraph
      - Keeps all facts, names, companies, dates unchanged
      - Does NOT summarise or shorten

    When only the title is available:
      - Writes a 400-500 word original article from the topic
    """
    if not GROQ_API_KEY:
        return None

    system = (
        "You are a precise news rewriter for The Tech Brief, an independent technology publication. "
        "You never summarise, never shorten, and never add commentary. "
        "You never reference or name any other publication, website, or news source."
    )

    if body_text and len(body_text.strip()) > 200:
        # Strip source name leakage from RSS body before sending to Groq
        clean_body = re.sub(
            r'\b(TechCrunch|Engadget|The Verge|CNET|Gizmodo|Kotaku|Electrek|InsideEVs|'
            r'VentureBeat|ZDNet|InfoWorld|The Register|SiliconAngle|Wired|Fortune|'
            r'Mashable|PCMag|TechRadar|Polygon|Eurogamer|IGN|PC\s*Gamer|'
            r'Dark Reading|CSO Online|TVB Europe|NewscastStudio|Crunchbase)\b',
            'an industry source', body_text, flags=re.IGNORECASE
        )
        # Also remove "appeared first on" attribution lines
        clean_body = re.sub(r'The post .+? appeared first on .+?\.', '', clean_body).strip()
        clean_body = re.sub(r'Read more at .+', '', clean_body, flags=re.IGNORECASE).strip()

        user = (
            f"Rewrite the following technology article for The Tech Brief's {category} section.\n\n"
            "STRICT RULES — follow exactly:\n"
            "1. Rewrite each paragraph separately. Do NOT merge or split any paragraphs.\n"
            "2. For every paragraph: preserve the exact number of sentences.\n"
            "3. Rewrite each sentence with different wording but identical meaning and facts.\n"
            "4. Keep ALL facts, company names, product names, people, numbers, and dates exactly as they are.\n"
            "5. Do NOT summarise. Do NOT shorten. The output must be the same total length as the input.\n"
            "6. Remove any phrases like 'according to', 'as reported by', 'via [source]', "
            "'appeared first on', or any reference to external publications.\n"
            "7. Return ONLY the rewritten article text. No headline, no byline, no labels.\n\n"
            f"ARTICLE TO REWRITE:\n{clean_body[:3500]}"
        )
        max_tok = 1400
    else:
        # Title-only: write a proper structured 400-word article
        user = (
            f"Write a 400-500 word original technology article for The Tech Brief's {category} section.\n\n"
            f"Topic: {title}\n\n"
            "Structure (4 paragraphs of ~100 words each):\n"
            "1. What is happening and why it matters right now\n"
            "2. Technical or industry background that explains the context\n"
            "3. Who is affected — businesses, consumers, developers — and how\n"
            "4. What changes as a result, and what to watch for next\n\n"
            "Rules: flowing original prose, no bullet points, no source names, no external quotes. "
            "Return ONLY the article body. No headline."
        )
        max_tok = 750

    text = _groq_post(system, user, max_tok)
    if text:
...
def rewrite_via_groq(title: str, category: str) -> str | None:
    return rewrite_rss_article(title, '', category)
...
def build_internal_article_page(title, editorial_text, category, cat_slug, cat_page, date_str, slug):
    """
    Build a full article HTML page.
    editorial_text may be:
      - A multi-paragraph rewritten article (500+ words, paragraphs separated by \\n\\n)
      - A single short fallback paragraph
    Either way, every paragraph is rendered as a proper <p> block.
    """
    image_url   = _pick_image(cat_slug, slug)
    canon_url   = f'{SITE_URL}/articles/{slug}.html'
    badge_color = _BADGE_COLORS.get(cat_slug, '#2563EB')
    ...
    # Split text into paragraphs and render each as a <p>
    raw_paras = [p.strip() for p in re.split(r'\n{2,}', editorial_text) if p.strip()]
    if not raw_paras:
        raw_paras = [editorial_text]

    first_para = raw_paras[0].replace('<','&lt;').replace('>','&gt;')
    rest_paras_html = ''.join(
        f'      <p>{p.replace("<","&lt;").replace(">","&gt;")}</p>\n'
        for p in raw_paras[1:]
    )
    safe_desc = first_para[:155]

    # Word count estimate for read time
    word_count = len(editorial_text.split())
    read_min   = max(3, word_count // 200)

    schema = json.dumps({
        "@context":"https://schema.org","@type":"Article",
        "headline": title, "image": image_url,
        "datePublished": date_str, "dateModified": date_str,
        "author": {"@type":"Organization","name":"The Tech Brief Editorial Team"},
        "publisher": {"@type":"Organization","name":"The Tech Brief","url":SITE_URL},
        "mainEntityOfPage": canon_url, "articleSection": category,
...
{schema}
  ...
  <link rel="stylesheet" href="../assets/styles.css">
  <style>
    .art-img{{width:100%;max-height:480px;object-fit:cover;border-radius:var(--radius);margin-bottom:36px;display:block;}}
    .art-lead{{font-size:19px;line-height:1.82;color:var(--ink-2);font-weight:300;margin-bottom:28px;padding-bottom:24px;border-bottom:1px solid var(--border);}}
    .art-body p{{font-size:16.5px;line-height:1.9;color:var(--ink);margin-bottom:22px;}}
    .art-meta{{display:flex;gap:16px;align-items:center;margin-bottom:28px;padding-bottom:18px;border-bottom:2px solid var(--border);flex-wrap:wrap;font-size:13px;color:var(--ink-3);}}
    .art-meta strong{{color:var(--ink);}}
    .rel-links a{{display:block;padding:10px 0;border-bottom:1px solid var(--border-2);color:var(--accent);font-size:15px;font-weight:500;}}
    .art-note{{background:var(--surface-2);border-left:4px solid var(--accent);padding:16px 20px;border-radius:0 var(--radius-sm) var(--radius-sm) 0;font-size:13.5px;color:var(--ink-2);line-height:1.6;margin-top:36px;}}
  </style>
</head>
<body>
<a class="skip-link" href="#main-content">Skip to main content</a>
...
<main id="main-content">
  <article class="page-wrap" style="max-width:760px;">
    ...
    <h1 style="font-family:var(--font-serif);font-size:clamp(27px,4vw,42px);line-height:1.2;margin-bottom:14px;color:var(--ink);">{safe_title}</h1>
    <div class="art-meta">
      <span>By <strong>The Tech Brief Editorial Team</strong></span>
      <span><time datetime="{date_str}">{pub_date}</time></span>
      <span>{read_min} min read · {word_count} words</span>
    </div>
    <img class="art-img" src="{image_url}" alt="{safe_title}" width="760" height="420" loading="eager">
    <div class="art-body">
      <p class="art-lead">{first_para}</p>
{rest_paras_html}    </div>
    <div class="art-note">
      <strong>Editorial Note:</strong> This article has been independently written and edited by The Tech Brief editorial team.
      <a href="../about.html" style="color:var(--accent);margin-left:4px;">About our process &rarr;</a>
    </div>
    <div style="border-top:2px solid var(--border);margin-top:36px;padding-top:22px;">
      <h3 style="font-family:var(--font-serif);font-size:19px;margin-bottom:14px;">Continue Reading</h3>
...
def build_category(category: str, urls: list, editorial_articles: list, cache: dict, rewrites_done: list) -> list:
    ...
    for url in urls:
        try:
            ...
            for e in feed.entries[:10]:
                ...
                # Extract the actual article body text from RSS entry
                # Priority: content > summary > description
                body_html = ''
                for c in (e.get('content') or []):
                    body_html = c.get('value') or c.get('content') or ''
                    if body_html: break
                if not body_html:
                    body_html = e.get('summary') or e.get('description') or ''
                body_text = clean_text(body_html)

                raw_items.append({
                    'title':     title,
                    'link':      link,
                    'body_text': body_text,   # ← actual article body from RSS
                    'image':     first_image(e),
                    'ts':        parse_time(e),
    ...
    for item in deduped:
        ...
        # Tier 1a: fresh cache — use cached rewrite
        if cached and _is_cache_fresh(cached):
            editorial_text = cached['editorial_summary']
            slug = cached['slug']

        # Tier 1b: Groq rewrite — pass actual body text
        elif rewrites_done[0] < MAX_REWRITES_PER_RUN and GROQ_API_KEY:
            body = item.get('body_text', '')
            print(f'    ✍  Groq [{rewrites_done[0]+1}] ({len(body)} chars): {item["title"][:50]}…')
            rewritten = rewrite_rss_article(item['title'], body, category)
            if not rewritten:
                rewritten = local_fallback_summary(item['title'], category, key)
            rewrites_done[0] += 1
            slug = f"rss-{key}"
            cache[key] = {
                'editorial_summary': rewritten,
                'slug':              slug,
                'title':             item['title'],
                'cat_slug':          cat_slug,
                'category':          category,
                'body_len':          len(rewritten),
                'cached_on':         datetime.now(timezone.utc).isoformat(),
            }
            editorial_text = rewritten

        # Tier 2: stale cache — still better than regenerating
        elif cached:
            editorial_text = cached['editorial_summary']
            slug = cached['slug']

        # Tier 3: local fallback — never blank, uses real topic title
        else:
            slug = f"rss-{key}"
            editorial_text = local_fallback_summary(item['title'], category, key)
            cache[key] = {
        ...
        # Write full article page to docs/articles/
        article_html = build_internal_article_page(
            item['title'], editorial_text, category, cat_slug, cat_page, iso_date, slug
        )
        with open(os.path.join(RSS_ARTICLES_OUT, f'{slug}.html'), 'w', encoding='utf-8') as f:
            f.write(article_html)

        # Card summary = first paragraph of the rewritten text (clean, readable)
        first_para = re.split(r'\n{2,}', editorial_text)[0].strip()
        card_summary = first_para[:280] + ('…' if len(first_para) > 280 else '')
    ...
    avg_words = sum(len(c['summary'].split()) for c in cards) // max(len(cards), 1)
    tier = f"Groq-rewrite" if GROQ_API_KEY else "local-fallback"
    print(f'  ✓ {cat_slug}.html — {len(cards)} articles [{tier}], ~{avg_words}w summaries, {len(cat_editorial)} deep-dives')
    return cards
